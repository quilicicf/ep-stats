...
module.exports = async ({ screenshot, appConfig, members, holidays, date }) => {
  ...
  const { global: { hits: hitsSize } } = screenshotSizeProfile;
  ...
  const tempFilePath = await readImage(imagePath)
    ...
    .then(threshold => writeImage(threshold, imageName));
