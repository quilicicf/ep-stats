...
const processFile = async (imageHeader, screenshot, screenshotProfile, members, holidays) => {
  const { filePath, fileName, date } = screenshot;
  if (imageHeader === 'TITAN HUNT') {
    log([ `Starting processing of ${fileName} from ${date.toISOString()} as Titan` ]);
    return processTitanInfoFile(filePath, fileName, screenshotProfile);

  } else if (imageHeader === 'ALLIANCE') {
    log([ `Starting processing of ${fileName} from ${date.toISOString()} as War` ]);
    return processWarInfoFile(filePath, fileName, screenshotProfile);

  } else if (imageHeader === 'CHAT') {
    log([ `Starting processing of ${fileName} from ${date.toISOString()} as Hits` ]);
...
module.exports = async (cache, screenshot, screenshotProfile, members, holidays) => {
  const { filePath, fileName } = screenshot;
  const imageHeader = await extractHeader(filePath, fileName, screenshotProfile);
  const result = await processFile(imageHeader, screenshot, screenshotProfile, members, holidays);
