...
const { SCREEN_TYPES: { WAR_HITS_TYPE, TITAN_HITS_TYPE }, GSHEET_DATE_FORMAT } = require('./conf/constants');
...
module.exports = async (imagePath, imageName, screenshotProfile, members, holidays, date) => {
  ...
  const sortedMembersScoreList = _(scoresByMembers)
    .map((memberScore, memberPseudo) => {
      ...
      if (shouldExcuseMember) {
        log([ `Member ${memberPseudo} was on holidays on ${date}. Setting score to 'N/A' for that day` ]);
      }
  ...
  return {
    type,
    date: formatDate(date, GSHEET_DATE_FORMAT),
    imageName,
    totalScore,
